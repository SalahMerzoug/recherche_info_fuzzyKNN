...
public class KNN {
	...
	public KNN(double [][] tfidfMatrix, int [] documentTopic, String [][]folderTopicArray, 
			String [] termColsArray, double [] idf, String [] filePathArray, String [] docNameArray){
		
		this.termColsArray = termColsArray;//tableau de mot 
		this.tfidfMatrix = tfidfMatrix;//matrice tf-idf
	...
	public void findNeighbor(int kValue, String testDocFilePath, int fuzzy) throws IOException {
		...
		testDocumentObj = createDocumentVector(testDocFilePath);//vectorisation
		
		
		double [] docTFIDFVector = buildTFVector(testDocumentObj);//creation tf-idf vector
		...
		//trouver le k avec le plus grand similarity
	...
	private void regKNN(HashMap <Integer, Integer> kNeighborMap, int kValue, int []similarDocIndex) {
		...
		for (Entry<Integer, Integer> entry : kNeighborMap.entrySet()) {
		  ...
		  config.result.add(new reslt(testDocumentObj.getNameOfDoc(), filePathArray[index], percent + "% "));
